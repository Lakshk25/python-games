...
to_learn = {}
try:
    data_frame = pd.read_csv('data/seven_days.csv')
...
# if word is not know place it in seven days to revise again after seven days
def seven():
...
def get_card():
    ...
    # custom ( for making seven days words revise)
...
def is_known():
    ...
    data.to_csv("data/seven_days.csv" , index=False)
    get_card()
...
unknown_btn = Button(image=cross_img , highlightthickness=0 , command=not_knowing_word)
